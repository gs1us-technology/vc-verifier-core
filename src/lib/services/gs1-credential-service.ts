import { documentLoader } from "../data-integrity/document-loader/documentLoader.js";
import { verifyCredential_Direct } from "../data-integrity/index.js";
...
export function gs1Validator(options: GS1CredentialValidationOptions = { credentialType: "DataIntegrityProof" }) {
    ...
    const getExternalCredential: externalCredential = async (url: string) : Promise<VerifiableCredential> => {
        ...
        if (validationOptions.credentialType === "JOSE") {
            ...
            const verifier_result = await verifyCredential({ type: jose_mime_type_vc, content: signedCredential},  jwk);
    ...
    const checkExternalCredential: verifyExternalCredential = async (credential: VerifiableCredential) : Promise<gs1RulesResult> => {
        ...
        // Data Integrity Proof Credentials required Proof Verification Before Validating GS1 Rules
        if (validationOptions.credentialType === "DataIntegrityProof") {
    ...
    async function checkGS1Credential(verifiableCredential: VerifiableCredential) : Promise <gs1RulesResult> {
        ...
        return await checkGS1CredentialWithoutPresentation(validatorRequest, verifiableCredential);
    }

    // Check if the Verifiable Presentation for any GS1 Credential and if so check the GS1 Credential Rules
     async function verifyGS1Credentials(verifiablePresentation: VerifiablePresentation) : Promise<gs1RulesResultContainer> {
        ...
        return await checkGS1CredentialPresentationValidation(validatorRequest, verifiablePresentation);
    }
