...
import { gs1RulesResult,  gs1RulesResultContainer, credentialResults, gs1CredentialValidationRule, verificationResult } from '@gs1us/vc-verifier-rules';
...
export async function createVerificationResult(verifiedResults: any, gs1RulesResultContainer: gs1RulesResultContainer | undefined = undefined): Promise<verificationResult> {
           ...
           const credentialResult: credentialResults = {
            ...
            if (credential.statusResult) {
                ...
                if (credential.statusResult.schemaValidation) {
                    const schemaValidationCheck = credential.statusResult.schemaValidation;

                    if (!schemaValidationCheck.verified) {
                        credentialVerifierTypeResult.JsonSchema = schemaValidationCheck.verified;
                    }
          
                    schemaValidationCheck.messages.forEach((message: gs1CredentialValidationRule) => {
                        credentialResult.credentialValidationRules?.push(message);
                    });
            ...
            if (credentialResult.error !== undefined && credentialResult.error?.length > 0) {
    ...
    gs1RulesResultContainer?.result?.forEach((gs1RulesResult: gs1RulesResult) => { 
        ...
        const gs1RulesResultResolveCredential = result.credentialResults.find((result) => result.credentialId === gs1RulesResult.credentialId);
    ...
    if (result.verifications) {
        ...
        if (typeof credentialVerifierTypeResult.JsonSchema !== 'undefined' ) {
            result.verifications.push({ title: "JsonSchema", status: credentialVerifierTypeResult.JsonSchema ? "good": "bad"})
        }
