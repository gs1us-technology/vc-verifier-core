...
import {
    GS1CredentialValidationOptions,
    gs1Validator,
} from '../services/gs1-credential-service.js'
...
import {
    ...
    verificationResult,
} from '@gs1us/vc-verifier-rules/dist/index.js'
...
function parseEnvelopeCredential(
    credential: transmute.EnvelopedVerifiableCredential
): string {
...
export async function verifyEnvelopeCredential(
    content: string,
    options: GS1CredentialValidationOptions = { credentialType: 'JOSE' }
    ...
    try {
        ...
        const verifiedResults = await verifyCredential(
        ...
        if (verifiedResults.verified) {
            ...
            const rulesCheckResult = await validator.checkGS1Credential(
        ...
        const claims = verifiedResults.verified
            ? jose.decodeJwt(content)
            : { id: 'unknown' }
        ...
        credentialValidationResult.result = await createVerificationResult(
            envelopeResult,
            gs1CredentialContainer
        )
    } catch (error) {
...
export async function verifyEnvelopePresentation(
    content: string,
    options: GS1CredentialValidationOptions = { credentialType: 'JOSE' }
): Promise<verificationResult> {
    ...
    const verifiedResults = await transmute
        ...
        .verify<
            transmute.VerifiablePresentationWithHolderObject &
                transmute.VerifiablePresentationOfEnveloped
        >({
    ...
    // Check GS1 Credential Validation Rules
    const validator = gs1Validator(options)
    const rulesCheckResult = await validator.verifyGS1Credentials(
        gs1PresentationToValidate
    )
...
function getCredentialResultError(
    errorMessage: string
...
export async function verifyCredential(
    request: transmute.RequestVerify,
    resolver: transmute.VerifierResolver
    ...
    try {
        ...
        if (verified) {
            ...
            verifiableCredentialResult.verified = isVerificationValid
                ? isVerificationValid
                : false
            verifiableCredentialResult.result = verified
            verifiableCredentialResult.statusResult = revocationStatus
        } else {
            verifiableCredentialResult = getCredentialResultError(
