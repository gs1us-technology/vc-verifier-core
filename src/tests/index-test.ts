...
import { showFailedTest, showFailedTestError, showPassedTest } from "./test-harness-log.js";
...
const TEST_HARNESS_ONLY_GS1 = process.argv[3] && process.argv[3] === '-gs1' ? true : false;
const TEST_HARNESS_ONLY_GS1_ERROR_ONLY = process.argv[3] && process.argv[3] === '-error' ? true : false;
const TEST_HARNESS_ONLY_GS1_JOSE = process.argv[3] && process.argv[3] === '-jose' ? true : false;
...
const filterTestHarness = function() : testHarness[] {
   ...
   if (!TEST_HARNESS_ONLY_GS1 && !TEST_HARNESS_ONLY_GS1_ERROR_ONLY && !TEST_RUN_SINGLE_TEST && !TEST_HARNESS_ONLY_GS1_JOSE) {
...
filterTestHarness().forEach(async function(testHarness){
  ...
  const funcName = testHarness.parameters ? testHarness.parameters.testName : testHarness.func.name;
  try {
    const testResult = await testHarness.func(testHarness.parameters); // Execute Test Function
    testResult.verified ? showPassedTest(funcName, testResult) : showFailedTest(funcName, testResult, true);
  } catch(error) {
